...
```bash
 ...
 rm -rf data/*                    # Затем удалить старую базу и выполнить скачивание, распаковку и удаление архива как это описано в командах выше
...
```
---  
## 🌟 15. Регистрация и управление валидатором
```bash
### 🔒 Получить ключ валидатора ###
arkeod tendermint show-validator # полученный ключ необходимо скпировать для выполения следующей команды
### ⚡ Создание файла validator.json,  будет необходим для выполнения следующей команды ###
nano validator.json # файл будет создан в той папке, где вы находитесь. Файл validator.json должен содержать:
{
  "pubkey": {
